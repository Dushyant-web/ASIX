...
// ("Flow field"). Zero dependencies: one WebGL canvas that fills its
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // Arrow rather than a `function` declaration: a hoisted declaration loses
    // the `canvas`/`gl` non-null narrowing established by the guards above, so
    // every use of them here would be a strict-mode type error. Only ever
    // invoked via requestRender(), which runs after this binding is
    // initialised, so the hoisting was never load-bearing.
